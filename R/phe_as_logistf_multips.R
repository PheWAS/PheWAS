phe_as_logistf_multips <-
  function(phe.gen, additive.genotypes=T,min.records=20,return.models=F,confint.level=NA, factor.contrasts=NA, alpha.para = 0.1, my.data, ...) {
    ...
    if(is.na(cov[1])) {stop("Covariates are required for propensity score")}
    covariates="PS"
    ...
    #check to make sure there are at least 8 exposed/unexposed inds.
    if(sum(table(d[gen])<8)>0){
      note=paste(note,"[Error: Not enough exposed inds for PS]")
    } else if(n_total<min.records) {
      ...
      set.seed(10)
      data.x <- as.matrix(d[,cov])
      glmnet.fit <- glmnet::cv.glmnet(x=data.x, y=d[,gen], family="binomial", standardize=TRUE, alpha=alpha.para)
      ps <- predict(glmnet.fit, data.x, s='lambda.min')[,1]
      d$PS=ps
      cov="PS"
      
      d=d %>% select(one_of(na.omit(unlist(c(phe,gen,cov)))))
