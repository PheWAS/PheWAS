...
expect_equal(phewas_ext(names(test_data_phenotype_1)[-1], genotypes = c('rsEXAMPLE'), covariates= 'sex', data = test_data_phenotype_2)$p[14], 0.90935256)})
#test_that('Base Case Works', {
...
test_that('Base Case Works', {
expect_equal(phewas_ext(names(test_data_phenotype_1)[-1], genotypes = c('rsEXAMPLE'), covariates= 'sex', data = test_data_phenotype_2, method = 'logistf')$p[14], 0.91162193)})