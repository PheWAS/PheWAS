         ...
         FUN=function(a){
           ...
           if(nrow(a)==0) {
             ...
             c=data.frame(phenotype=phenotype,snp=snp,adjustment=adjustment,
                          beta=NA, OR=NA,SE=NA,p=NA,type=NA,
                          n_total=sum(a$n_total,na.rm=TRUE),
                          n_cases=sum(a$n_cases,na.rm=TRUE),
                          n_controls=sum(a$n_controls,na.rm=TRUE),
                          HWE_p.min=NA,allele_freq=NA,
                          n_no_snp=NA,k_studies=0,tau2=NA,I2.percent=NA,
                          Q=NA,Q.df=NA,Q.p=NA,beta.fixed=NA,OR.fixed=NA,
                          SE.fixed=NA,p.fixed=NA,beta.random=NA,
                          OR.random=NA,SE.random=NA,p.random=NA)
